...
const createTransporter = () => {
  ...
  let emailUser = functions.config().email?.user;
  let appPassword = functions.config().email?.password;
  let emailTo = functions.config().email?.to;

  // If not set, use hardcoded values (temporary solution)
  if (!emailUser) {
  ...
  if (!emailUser || !appPassword || appPassword === 'your-16-character-app-password') {
...
const sendContactEmail = async (formData) => {
  ...
  const recipientEmail = functions.config().email?.to || 'harham0210@gmail.com';
  const transporter = createTransporter();

  const mailOptions = {
    from: `"Quran Academy Website" <${functions.config().email?.user}>`,
    to: recipientEmail,
    replyTo: email,
...
const sendTrialBookingEmail = async (formData) => {
  const {
    parentName,
    email,
    phone,
...
exports.bookTrial = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Headers', 'Content-Type');
  res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
